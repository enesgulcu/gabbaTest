...
const handler = async (req, res) => {
  ...
  const checkData = async (data, productId = null) => {
    try {
      ...
      const processedFeatures = data.productFeatures.filter((item) => {
        if (
          item.feature === 'Extra' &&
          (!item.extraValue || item.extraValue.trim() === '')
        ) {
          return false;
        }
        if (
          item.feature === 'Image' &&
          (!item.imageValue || item.imageValue.trim() === '')
        ) {
          return false;
        }
  ...
  try {
    if (req.method === 'POST') {
      ...
      //silme işlemi için gelen veriyi sileriz.
        ...
        try {
          ...
          if (process == 'deleteProduct') {
            ...
            const deleteProductFeatureData = await deleteDataByMany(
              'ProductFeature',
              {
                productId: data,
              }
            ...
            const deleteProductData = await deleteDataByAny('Products', {
              id: data,
            });
          ...
          if (process == 'deleteFeature') {
            ...
            // }
            let deleteProductFeatureData = await deleteDataByMany(
              'ProductFeature',
              data
            );
            ...
            if (!deleteProductFeatureData.count) {
              ...
              deleteProductFeatureData = await deleteDataByAny(
                'ProductFeature',
                { id: data.featureId }
              );
            }
            ...
            return res.status(200).json({
              status: 'success',
              message: 'Ürün özelliği başarıyla silindi.',
            });
          }
        } catch (error) {
      ...
      // yeni veri oluşturuyoruz.
        ...
        return res.status(200).json({
          status: 'success',
          data: checkedData,
          message: createProducts.message,
        });
      } else {
    ...
    if (req.method === 'GET') {
      ...
      if (productId) {
        const response = await getDataByUnique('Products', {
          id: productId,
        });
      ...
      if (extraId) {
        const response = await getDataByUnique('ProductFeature', {
          id: extraId,
        });
        return res.status(200).json({ status: 'success', data: response });
      }
      ...
      return res.status(200).json({
        status: 'success',
        data: { createProducts, productFeatures },
        message: createProducts.message,
      });
    }
  } catch (error) {
