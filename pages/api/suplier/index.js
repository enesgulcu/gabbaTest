...
const handleDelete = async (req) => {
  ...
  if (req.body.type === 'one') {
    data = await deleteDataByMany('Supplier', {
      id: {
        equals: req.body.id,
      },
    });
  ...
  if (req.body.type === 'all') {
    data = await deleteDataAll('Supplier');
  }
...
const handler = async (req, res) => {
  try {
    if (req.method === 'GET') {
      const financialManagementSpecial = await getAllData('Supplier');
      ...
      const financialManagementSpecial = await createNewData('Supplier', {
        ...req.body.newData,
      });
      if (!financialManagementSpecial || financialManagementSpecial.error) {
        throw 'Bir hata oluştu. Lütfen teknik birimle iletişime geçiniz. XR09KY4';
